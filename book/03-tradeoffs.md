...
表里有两个词先记个大概：**三段瀑布**指工具执行前、执行中、执行后这三处都能挂监听器（`tools/pre-execute` / `execute` / `post-execute`）；**单调守卫**是夹在前两处之间的一道独立的关，只能拒绝、不能放行——正因为只能往一个方向走，才叫「单调」，加再多守卫也不会互相翻案。两个都在第 10 章展开。
...
**图 3-1：扩展点的位置决定了扩展能力的上限**。内核派那边你只能碰到预留的洞；插件树派这边，你写的插件和 `agent-loop` 在同一棵树上，地位相同
