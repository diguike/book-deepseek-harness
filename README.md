...
**模型 API key 不是必需的，但有更好。** 第 1、11、15 章的核心数据来自 DeepSeek 官方端点——端到端事件流、十变量的前缀缓存实验、六轮调用的真实用量轨迹，都带缓存命中数字。没有 key 也能跟着做：`examples/mock-llm-server` 把本机 `claude` CLI 包成 OpenAI 兼容端点，两条 patch 接上去。
